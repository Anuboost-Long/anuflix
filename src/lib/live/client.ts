import type { LiveMatch, LiveMatchesPage, LiveSource, LiveSport, LiveStream } from "@/lib/live/types"
...
async function streamed<T>(path: string, revalidate: number | false): Promise<T> {
  ...
  if (!response.ok) {
    throw new StreamedError(`Streamed request failed with status ${response.status}.`, response.status)
  }
...
export async function getLiveMatchesPage({
  ...
  const filtered = matches
    ...
    .filter((match) => !normalizedQuery || match.title.toLowerCase().includes(normalizedQuery))
    .sort((left, right) => {
