    ...
    <>
      <div className="sticky top-20 z-30 border-y border-border bg-background/92 px-[clamp(1.25rem,4vw,4.5rem)] py-4 backdrop-blur-xl">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <label className="flex h-11 min-w-0 flex-1 items-center gap-3 rounded-lg border border-border bg-surface px-3 focus-within:border-brand-bright focus-within:shadow-[0_0_0_3px_rgba(59,130,246,.12)]">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search teams, leagues, or events..." className="h-full min-w-0 flex-1 bg-transparent text-sm text-text-primary outline-none placeholder:text-text-subtle" />
          </label>
      ...
      <div className="px-[clamp(1.25rem,4vw,4.5rem)] py-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <span className="text-[10px] font-semibold tracking-[.16em] text-brand-light uppercase">Updated throughout the day</span>
            <h2 className="mt-1 text-2xl font-bold text-text-primary">{sport === "all" ? "All matches" : sports.find(({ id }) => id === sport)?.name}</h2>
          </div>
        ...
        {items.length ? (
          <>
            <div className={clsx("grid grid-cols-1 gap-4 transition-opacity sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4", loading && page === 1 && "opacity-45")}>
              {items.map((match) => <LiveMatchCard key={match.id} match={match} live={live.has(match.id)} />)}
            </div>
            {more ? <div className="mt-10 flex justify-center"><button type="button" onClick={loadMore} disabled={loading} className="h-11 rounded-lg border border-border-strong bg-surface px-6 text-sm font-semibold text-text-primary transition-colors hover:bg-surface-hover disabled:cursor-wait disabled:opacity-60">{loading ? "Loading matches..." : "Load more matches"}</button></div> : null}
          </>
        ) : (
          <div className="border-y border-border py-20 text-center"><h3 className="text-xl font-bold text-text-primary">{loading ? "Finding matches..." : "No matches found"}</h3><p className="mt-2 text-sm text-text-secondary">{loading ? "Checking the latest schedule." : "Try another sport or search term."}</p></div>
        )}
        {error ? <p role="alert" className="mt-5 text-center text-sm text-red-300">{error}</p> : null}
      </div>
    </>
  )
}
